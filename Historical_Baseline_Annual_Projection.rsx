...
lc_lu1<-lc_lut[1,]$LUT_NAME
get_from_rdb(symbol=paste(lc_lut1), filebase=paste(data_dir, "land_use_cover", sep=""))
lc_lut<-list_of_data_luc[which(list_of_data_luc$PERIOD==t2),]
lc_lu2<-lc_lut[1,]$LUT_NAME
get_from_rdb(symbol=paste(lc_lut2), filebase=paste(data_dir, "land_use_cover", sep=""))
# new directory 
...
},error=function(e){cat("Failed to save the dbf", "\n")})
...
eval(parse(text=(paste("d1<-", lc_lu1))))
eval(parse(text=(paste("d2<-", lc_lu2))))
d1<-d1[which(d1$ID!=0),]
d2<-d2[which(d2$ID!=0),]
