...
init.YEAR<-year(as.Date(init.date, "%d/%m/%Y"))
final.YEAR<-year(as.Date(final.date, "%d/%m/%Y"))
year.interval<-final.YEAR-init.YEAR+1
total.data.row<-year.interval*365
...
eval(parse(text=(paste("sub.out.params<-as.data.frame(cbind(YEAR,DAY,precip.sub.out,pet.sub.out,et.sub.out,perc.sub.out,surq.sub.out,gwq.sub.out,latq.sub.out,wyld.sub.out,syld.sub.out))", sep=""))))
eval(parse(text=(paste("sub.out.params.",subas,"<-head(sub.out.params,",total.data.row,")",sep=""))))
}
...
outflow.rch.out<-head((((rch.out[rch.out$V2==end.rch,7])*24*3600)/(tot.area[1]*(10^6)))*1000,total.data.row); #conversion from cms to mm
sed.rch.out<-head(rch.out[rch.out$V2==end.rch,10], total.data.row)
...
basin.a<-as.data.frame(cbind(YEAR,DAY,head(tot.area,total.data.row),(basin.precip/NSUB),(basin.surface/NSUB), (basin.base/NSUB), (basin.soil/NSUB),outflow.rch.out, sed.rch.out))
...
rain.above<-as.data.frame(1:365, header=T)
...
mdebit.above<-as.data.frame(1:365, header=T)
