...
F_directory<-paste(Directory_of_factors, "/Factors", sep="")
setwd(F_directory)
test<-read.csv("singlestep_2005-2010.csv", header=TRUE, sep=",")

test[4]<-NULL
colnames(test)[1]<-"From*"
colnames(test)[2]<-"To*"
colnames(test)[3]<-"Rate"
test<-edit(test)
write.csv(test, file="singlestep_2005-2010.csv", row.names=FALSE)
...
rtffile <- RTF("LUMENS_SCIENDO_Scenario_report.lpr", font.size=9)
...
command<-paste("start ", "winword ", Directory_of_factors, "/LUMENS_SCIENDO_Scenario_report.lpr", sep="" )
shell(command)
